...
let rec inline ppf = function 
  ...
  | Text (_, s) when s = "***" || s = "___" || s = "---" -> pf ppf "    %s" (escape_text s)
  ...
  | Hard_break _ -> pf ppf "  @ "
  | Soft_break _ -> pf ppf "@ "
  | Link (attrs, { label; destination; title = None }) -> pf ppf "[%a](%s)%a" inline label (escape_link_destination destination) attributes attrs
  | Link (attrs, { label; destination; title = Some title }) -> pf ppf "[%a](%s \"%s\")%a" inline label (escape_link_destination destination) title attributes attrs
  | Image (attrs, { label; destination; title = None }) -> pf ppf "![%a](%s)%a" inline label (escape_link_destination destination) attributes attrs
  | Image (attrs, { label; destination; title = Some title }) -> pf ppf "![%a](%s \"%s\")%a" inline label (escape_link_destination destination) title attributes attrs
...
and block ?(tight=false) ?(list=None) ppf = function 
  | Thematic_break _ -> ( match list with
    | Some '-' -> pf ppf "***\n"
    | Some _ | None -> pf ppf "---\n"
  )
  | Paragraph (_, il) -> if tight then pf ppf "%a" inline il else pf ppf "%a@ " inline il
  | List (_, typ, spacing, blockss) -> 
      let tight = spacing = Tight in 
      let elt typ ppf = match typ with 
        | Bullet c -> pf ppf "%c @[<v0>%a@]" c (pp_list (block ~tight ~list:(Some c)))
        | Ordered (i, c) -> pf ppf "%i%c @[<v0>%a@]" i c (pp_list (block ~tight ~list:(Some c)))
      in 
    pf ppf "@[<v0>%a@]" (pp_list (elt typ)) blockss
  | Heading (attrs, size, il) -> pf ppf "%s %a%a" (String.make size '#') inline il attributes attrs
...
and attributes ppf attrs = 
  if List.length attrs = 0 then () else 
  ...
  in 
  let split_attrs = 
    List.(fold_left (fun acc (k, v) -> (rev (map (fun v' -> (k, v')) (String.split_on_char ' ' v))) @ acc) [] attrs) |> List.rev
  in
