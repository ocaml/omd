...
let preprocess l =
  List.fold_left (fun r e -> e r) l !preprocess_functions
...
let register_code_stylist_of_program x =
  try
    ...
    code_stylist#register ~lang:(String.sub x 0 i) (code_stylist_of_program (String.sub x (i+1) (String.length x - (i+1))))
...
let patch_html_comments l =
  let htmlcomments s =
    ...
    for i = 4 to String.length s - 4 do
      match s.[i] with
      | '-' as c ->
          if (i > 4 && s.[i-1] = '-') || (i < String.length s - 5 && s.[i+1] = '-') then
...
let tag_toc l =
  ...
  let x =
    object (self)
      ...
      val remove = fun e md -> visit (function X(v) when v==e-> Some[] | _ -> None) md
  ...
  in
  let rec loop = function
    | Delim (1, Star) :: Word "Table" :: Delim (1, Space) :: Word "of" :: Delim (1, Space) ::
      Word "contents" :: Delim (1, Star) :: tl ->
        Tag("tag_toc",
            object
              method parser_extension r p l = Some(X(x)::r,p,l)
              method to_string = ""
            end) :: loop tl
    | e :: tl ->
        e :: loop tl
...
let main () =
  ...
  let input_files = if !input = [] then [stdin] else List.rev_map (open_in) !input in
  ...
  List.iter (fun ic ->
      ...
      with End_of_file ->
        ...
        let o1 = (* make either TOC or paragraphs, or leave as it is *)
          if !otoc then
            Omd.toc ~start:!toc_start ~depth:!toc_depth parsed
          else
            Parser.make_paragraphs parsed
        in
        let o2 = (* output either Text or HTML, or markdown *)
          if !notags then
            to_text o1
          else if !omarkdown then
            to_markdown o1
